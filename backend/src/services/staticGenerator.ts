 ...
 * 生成知识图谱静态数据
 */
export async function generateKnowledgeGraphStatic(): Promise<void> {
 ...
 * 获取静态文件的生成时间
 */
export async function getStaticFileInfo(): Promise<{ generatedAt: string | null; exists: boolean }> {
  const filePath = path.join(STATIC_DIR, 'knowledge-graph.json');

  if (!existsSync(filePath)) {
    return { generatedAt: null, exists: false };
  }

  try {
    const content = await import(filePath, { assert: { type: 'json' } });
    return { generatedAt: content.default.generatedAt, exists: true };
  } catch {
    return { generatedAt: null, exists: true };
  }
}
